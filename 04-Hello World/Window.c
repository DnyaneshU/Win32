...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)//pahile parameter: kuthe click zala ahe he OS global hwnd table madhe map karte and te tya msg cha ball madhe pass karte. Dusra parameter: nakki msg kasa ahe: Left button click: WM_LBUTTONDOWNl;Right button click: WM_RBUTTONDOWN;Key down: WM_KEYDOWN. All these are macros having unsigned int valed. So left click asel tar te jata hya second parameter. So all window are UINT type.Msg struct che pahile 4 members he WndProc che 4 parameters ahet.So he 4 members parameters mhanun pass hoto WndProc madhe.
{
	...
	TCHAR str[] = TEXT("Hello to the world from WinDev-2024");
	//code
	...
	case WM_PAINT://fakt painting related code asla pahijel itha.He sagle aple 8 cases madhe yenar ahet je apan lijla ahet.
		...
		//all the 3 functions below are concerned with the color of the text.
		SetBkColor(hdc, RGB(0, 0, 0));//We already had a brush that had set our color to black/white accordingly.so this sets background color of only my text and not my window.Set text bg color to black.Tell the painter hdc to set bg color of our text to black.2nd parameter mhanje toh rang ahe.
		SetTextColor(hdc, RGB(0, 255, 0));//tell the painter hdc to set your text color to green(255).
		DrawText(hdc, str, -1, &rect, DT_CENTER | DT_VCENTER | DT_SINGLELINE);//tell the painter hdc 4 things:1. Which text to draw(str).2. what is the length of the text to draw(-1).str is tchar type so its distnace would be wchar string length. So -1 tells us akha string chi length ghe.So it is a method to tell akhi string length.3. in which client area rectangle should i draw the text(&rect) and give its coordinates.4.How should i draw the text: DT_CENTER mhanje draw text and maza text madhe asla pahijel. DT_VCENTER mhanje vertically madhe ali pahijel and maza text single line madhe asla pahijel-DT_SINGLELINE.
	...
	/*case WM_SIZE:
		GetClientRect(hwnd, &rect);//hyala bolavnayche donach jaga asle pahijel: WM_SIZE and WM_PAINT.
		break;*/
